...
class DACS(UDADecorator):
    ...
    def __init__(self, **cfg):
        ...
        model_type = "vit_t"
        sam_checkpoint = "/root/autodl-tmp/pretrain_weight/mobile_sam.pt"
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, target_img,
        ...
        if pseudo_label.ndim == 4 and pseudo_label.shape[1] == 1:
            pseudo_label = pseudo_label.squeeze(1)
        imgs_match = torch.cat([img, target_img], dim=0)  # [B, 3, H, W]
        img_metas_match = img_metas + target_img_metas
        gt_semantic_segs_match = torch.cat([gt_semantic_seg, pseudo_label], dim=0).long()  # [B, H, W]

        #tarin the source/targe domain
        target_losses = self.get_model().forward_train(
            imgs_match, img_metas_match, gt_semantic_segs_match.unsqueeze(1), return_feat=True,match=True
        )
        target_losses.pop('features')

        warmup_steps=1000
        step_factor = min(1.0, self.local_iter / float(warmup_steps)) 
        for key in target_losses.keys():
