...
def get_rcs_class_probs(data_root, temperature, sample_class_stats):
    # with open(osp.join(data_root, 'sample_class_stats.json'), 'r') as of:
    with open(osp.join(data_root, sample_class_stats), 'r') as of:
    ...
    def __init__(self, source, target, cfg):
        ...
        if self.rcs_enabled:
            ...
            self.rcs_classes, self.rcs_classprob = get_rcs_class_probs(
                cfg['source']['data_root'], self.rcs_class_temp, cfg.get('sample_class_stats'))
            ...
            with open(
                    osp.join(cfg['source']['data_root'],
                            cfg.get('samples_with_class')), 'r') as of:
            ...
            for i, dic in enumerate(self.source.img_infos):
                ...
                # if isinstance(self.source, CityscapesDataset) or isinstance(self.source, CityscapesDataset_13) or isinstance(self.source, SynPASS_11) or CityscapesDataset_openpartset:
                if isinstance(self.source, (CityscapesDataset, CityscapesDataset_13, SynPASS_11_sun, SynPASS_11_weather)):
                    file = file.split('/')[-1]
